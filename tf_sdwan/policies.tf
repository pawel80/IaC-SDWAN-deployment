...
resource "sdwan_custom_control_topology_policy_definition" "hub_spoke_filter_traffic_v1" {
  ...
  description    = "Force HUB to SPOKES plus traffic filtering"
...
resource "sdwan_centralized_policy" "hub_spoke_policy_v1" {
  ...
  description = "Centralized Policy: HUB-SPOKE and traffic filtering"
  definitions = [
    # {
    ...
    # },
    {
      id = sdwan_custom_control_topology_policy_definition.hub_spoke_filter_traffic_v1.id
      type = "control"
      entries = [
        {
          site_list_ids = [sdwan_site_list_policy_object.spokes_v1.id]
          # vpn_list_ids  = [sdwan_vpn_list_policy_object.vpns_v1.id]
          direction     = "out"
        }
      ]
